...
import { BookMarked, Award, User, Clock } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
          <BookMarked className="w-6 h-6 text-blue-600" /> Enrolled Academic Subjects
        </h2>
        <p className="text-xs text-slate-500 mt-1">Course modules, credit allocations, and faculty professors for this term.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {loading ? (
          <div className="col-span-3 text-center py-10 text-slate-400">Loading subjects...</div>
        ) : subjects.length === 0 ? (
            ...
            <div key={sub._id} className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between">
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
                <span className="flex items-center gap-1">
                  <User className="w-3.5 h-3.5 text-slate-400" /> {sub.facultyId?.userId?.name || 'Instructor'}
                </span>
                <span className="font-semibold text-slate-800">Semester {sub.semester}</span>
              </div>
            </div>
          ))
        )}
